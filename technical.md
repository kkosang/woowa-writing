...
2. 캐시에 비트맵이 있는 경우(cache hit), 이미지 후 처리 단계로 진행한다.

3. 캐시에 비트맵이 없는 경우(cache miss), 디스크 캐시에서 이미지를 가져온다.

4. 디스크에 캐시가 있는 경우(cache hit), 디코딩 후 비트맵을 메모리 캐시에 저장하고, 이미지 후처리 단계를 진행한다.

5. 디스크에 캐시가 없는 경우(cache miss), 이미지를 외부 (네트워크,리소스)에서 다운로드 한다.
...
네트워크 연결을 통해 한 번 다운로드된 이미지를 **메모리나** **디스크에** 저장하여, 동일한 이미지를 다시 사용할 때 네트워크를 통해 불러오지 않고 저장된 데이터를 사용해 빠르게 로드할 수 있다.
...
Glide, Coil, Picasso 같은 이미지 로딩 라이브러리를 사용하면 캐싱 및 로딩을 손쉽게 처리할 수 있다.
...
- 이미지 목록들을 부드럽고 빠르게 스크롤링 가능하다.
- GIF와 비디오 썸네일을 포함한 다양한 이미지 형식을 지원한다.
- 비동기 이미지 로딩 및 라이프사이클을 자동으로 관리할 수 있다.
- 자동으로 메모리 및 디스크 캐싱을 처리할 수 있다.
...
- Kotlin DSL을 활용하여 코드가 간결하고 읽기 쉽다.
- Coroutines을 이용하여 비동기 이미지 로딩을 쉽게 처리할 수 있다.
- 최대 2000개의 method 정도로 가볍게 사용할 수 있다.
...
- 간단한 API를 제공하여 이미지 로드가 편한다.
- 메모리 및 디스크 캐싱을 자동으로 처리하여 성능을 최적화한다.
- 이미지 리사이징, 회전, 자르기 등의 간단한 처리가 쉽다.
...
위 테스트의 결과가 부정확할 수 있지만, 모든 라이브러리가 Test1보다 Test2에서 안정적인 메모리 사용량을 보여주었습니다.
